...
export const mockSkinScore: SkinScoreSnapshot = {
  ...
  overallScore: 78,
  parts: [
    { part: 'forehead', label: '이마', grade: '양호', moisture: 72, elasticity: 68 },
    { part: 'glabella', label: '미간', grade: '보통', moisture: 60, elasticity: 64 },
    { part: 'eyeArea', label: '눈가', grade: '보통', moisture: 55, elasticity: 58 },
    { part: 'cheek', label: '볼', grade: '양호', moisture: 75, elasticity: 70 },
    { part: 'lips', label: '입술', grade: '건조', moisture: 40 },
...
export const mockRecommendations: Recommendation[] = [
  ...
  },
  {
    ...
    explanation:
      '초미세먼지(PM2.5) 노출은 모공에 침투해 활성산소를 생성하고 콜라겐 분해를 촉진할 수 있다는 관찰 연구 결과가 있습니다. 오늘 PM2.5 농도를 고려해 이중 세안으로 잔여 오염물질 제거를 권장합니다.',
  ...
  },
  {
    ...
    explanation:
      '최근 2주간 오존 농도가 높았던 날 다음날 회원님의 피부 수분 지표가 낮아지는 경향이 관찰되었습니다. 이는 개인 표본 기반의 통계적 관찰이며 확정적 인과관계가 아닙니다.',
  ...
  },
  {
    ...
    explanation:
      '오늘 측정된 입술 부위 수분 수치가 40으로 낮게 측정되었습니다. 낮은 습도와 함께 장벽 손상 위험이 높아질 수 있어 보습 밤 사용에 도움될 수 있습니다.',
...
// 백엔드에 아예 접속이 안 될 때(getWeather도 실패)만 쓰는 최후 폴백 — 서버 쪽 FALLBACK_WEATHER_PRODUCTS와 동일한 내용
export const mockWeatherProducts: Product[] = [
  {
...
export const mockPersonalPattern: PersonalPattern = {
  id: 'pattern-1',
  title: 'PM2.5 노출과 피부 수분도 변화',
  grade: 'C',
  description: '초미세먼지 농도가 높았던 날 이후 피부 수분 지표가 낮아지는 경향이 관찰됩니다.',
  series: Array.from({ length: 14 }).map((_, i) => ({
    date: new Date(Date.now() - (13 - i) * 86400000).toISOString(),
    skinMetricValue: 60 + Math.round(Math.sin(i / 2) * 10),
    environmentValue: 20 + Math.round(Math.cos(i / 3) * 15),
  })),
};

export const mockPatternProgress = {
  collectedDays: 12,
  requiredDays: 21,
};
