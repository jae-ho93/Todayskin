...
export class RecommendationDto {
  ...
  @ApiProperty({ example: '초미세먼지 노출은 모공에 침투해 활성산소를 생성할 수 있다는 관찰 연구 결과가 있습니다.' })
  explanation!: string;

  @ApiPropertyOptional({ example: '통계적 관찰 - 확정적 인과관계 아님' })
  observationalNote?: string | null;

  @ApiProperty({ type: [String], example: ['세라마이드', '판테놀'] })
  ingredientTags!: string[];

  @ApiProperty({ type: [String], example: [], description: '연결된 제품 ID 목록 (호환용 빈 배열)' })
  relatedProductIds!: string[];

  @ApiPropertyOptional({ enum: ['외출 후', '자기 전', '언제든'], description: '언제 적용하면 좋은지' })
