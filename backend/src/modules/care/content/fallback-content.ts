 ...
 *
 * fallback은 postProcess를 거치지 않으므로(LIVE 생성 전 즉시 응답) phase를 여기서부터
 * 이미 화면이 기대하는 고정값으로 맞춰둔다 — weather/skin/combined는
 * "외출 후(세안 후)"/"자기 전", morning은 "자기 전" 없이 "외출 전"/"외출 중".
 */
const GENERIC_FALLBACK_ROUTINE = [
  {
    phase: CARE_FIXED_PHASES[0],
    step: '순한 세안 후 보습',
    ingredient: '히알루론산',
    amount: '500원 동전 크기',
...
const MORNING_FALLBACK_ROUTINE = [
  {
    phase: '외출 전',
    step: '순한 세안 후 보습',
    ingredient: '히알루론산',
  ...
  },
  {
    ...
    reason: '자외선 차단은 계절·수치와 무관하게 매일 챙기면 도움될 수 있어요.',
    detail:
      '아침에 한 번 발랐어도 2~3시간 지나면 효과가 줄어들어요. 특히 낮 시간에 오래 밖에 있는 날엔 ' +
      '틈틈이 덧발라줘야 처음 바른 만큼의 차단 효과가 유지돼요.',
    evidence: null,
  },
];
...
export function fallbackCarePlan(careType: CareType): CarePlanDto {
  return {
    careType,
    routine: careType === 'morning' ? MORNING_FALLBACK_ROUTINE : GENERIC_FALLBACK_ROUTINE,
    products: [],
    medicalDisclaimer:
      careType === 'weather'
        ? null
