...
**products의 개수는 routine에서 ingredient가 채워진 단계 수와 반드시 같아야 합니다 — 하나도 빠지면
안 됩니다.** 예를 들어 routine에 "히알루론산", "세라마이드", "시어버터", "징크옥사이드" 네 성분이
나오면 products도 정확히 네 개, 그 네 성분을 각각 포함하는 실제 제품이어야 합니다. 이건 "가능하면"이
아니라 반드시 지켜야 하는 개수입니다.

한 번의 web_search로 정확히 그 성분명이 들어간 제품이 안 나오면, 검색어를 바꿔서(예: 성분명 대신
"그 성분 위주의 [제품 카테고리]") 다시 찾아보고, 그래도 못 찾으면 그 성분이 주요 성분으로 포함된
비슷한 제품이라도 반드시 채우세요 — 그 단계에 대응하는 product를 아예 비워두지 마세요.

routine과 무관하게 따로 노는 제품을 넣지 마세요. reason 첫 문장에 어느 단계·성분을 위한 것인지
명시하세요 (예: "위 세럼 단계의 히알루론산이 포함된 제품이에요"). 같은 단계에 제품 여러 개를 몰아주지
말고, routine의 서로 다른 단계에 하나씩 정확히 대응시키세요.`;
...
products는 routine에서 ingredient가 채워진 단계 수만큼 web_search로 실제 존재를 확인한 제품을
담고(성분마다 하나씩), 각 제품이 오늘 날씨의 어떤 수치 때문에 도움이 될 수 있는지 reason에 쓰세요.
...
routine에는 각 단계에서 어떤 성분(ingredient)을 얼마나(amount) 바르는지 구체적으로 담으세요.
products는 routine에서 ingredient가 채워진 단계 수만큼 web_search로 실제 존재를 확인한 제품을
담고(성분마다 하나씩), 각 제품이 사용자의 오늘 피부 상태에 왜 도움이 될 수 있는지 reason에 쓰세요.
...
routine에는 각 단계에서 어떤 성분(ingredient)을 얼마나(amount) 바르는지 구체적으로 담으세요.
products는 routine에서 ingredient가 채워진 단계 수만큼 web_search로 실제 존재를 확인한 제품을
담고(성분마다 하나씩), 오늘 날씨와 피부 상태를 함께 근거로 reason에 쓰세요.
...
성분(ingredient)을 얼마나(amount) 바르는지 구체적으로 담으세요. products는 routine에서
ingredient가 채워진 단계 수만큼 web_search로 실제 존재를 확인한 제품을 담고(성분마다 하나씩),
오늘 날씨와 어젯밤 피부 상태를 함께 근거로 reason에 쓰세요.
