...
describe('GeminiClient', () => {
  ...
  it('MOCK_GEMINI=true 시 추천 mock 응답 반환', async () => {
    const client = new GeminiClient(makeConfig({ MOCK_GEMINI: 'true' }));
  ...
  it('MOCK_GEMINI=true 시 날씨 기반 제품 mock 응답 반환 (3개)', async () => {
    const client = new GeminiClient(makeConfig({ MOCK_GEMINI: 'true' }));
  ...
  it('키 없음 시 GeminiUnavailable (추천)', async () => {
    const client = new GeminiClient(makeConfig({ MOCK_GEMINI: 'false' }));
    await expect(
