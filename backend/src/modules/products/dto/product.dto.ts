...
export class ProductDto {
  ...
  @ApiProperty({ type: [String], example: ['징크옥사이드', '나이아신아마이드'] })
  matchedIngredients!: string[];

  @ApiProperty({ enum: ProductCategory, example: ProductCategory.BARRIER })
  category!: ProductCategory;

  @ApiPropertyOptional({ description: '연결된 추천 ID (카탈로그 제품용, 호환)' })
  recommendationId?: string | null;

  @ApiPropertyOptional({ description: '날씨 기반 제품의 근거 설명' })
  reason?: string | null;

  @ApiPropertyOptional({ enum: ['세안 후', '외출 전', '외출 후'], description: '하루 중 이 제품을 쓰면 좋은 상황' })
