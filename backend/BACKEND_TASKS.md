...
> 프론트 범위에서 완료된 작업(N15 캘린더 연결, N18 세션 수명, N19 설정 화면)은
> 아래 `프론트 범위 완료 기록`에 보존한다. 미완료 프론트 작업(N23 EAS 배포)은 프론트 팀 문서에서 관리한다.
...
- [ ] 구독(프리미엄) 화면 — 현재 정적 표시만, 결제·권한 로직 없음 (범위 별도 결정 — 미완료)
