...
>
> #### 2026-08-13 후속 2차 — 배포 준비 웨이브 (PR #167~#172)
>
> 같은 날 오후, 리뷰의 잔여 실행 항목을 2차 웨이브로 반영 완료:
>
> - **D-01 결정 완료 → D-02 적용**: 질환·여드름 리포트를 **기본 숨김 + 설정 "실험실(베타)"
>   옵트인**으로 전환(F79, 15장 B-3 권고 채택). 규제 경계 노출이 기본 상태에서 사라졌다.
> - **Product TOP2 주간 요약**(F80, 18·19장) · **UX TOP2·3·9 측정 플로우 신뢰**(F81 —
>   분석 단계 표시+취소, 첫 측정 기준점, 재측정 CTA) · **UX TOP7·8·10 상태 완성도**(F82 —
>   홈 스켈레톤·오프라인 배너·패턴 잠금 미리보기) · **Pretendard 폰트**(F83, 8·9장).
> - **N54 배포 준비 마감**: `linux/amd64` 이미지 2종 빌드·프로덕션 부팅 스모크 실측 —
>   이 과정에서 **프로덕션 이미지의 Prisma client 누락(부팅 즉시 크래시) 버그를 발견·수정**했다.
>   36장이 지적한 "배포 검증 부재" 리스크가 실측으로 해소됐고, 배포 당일 체크리스트와
>   장애 런북이 `docs/guides/DEPLOYMENT.md`에 추가됐다.
> - 이 시점 기준 남은 것: **N16(AWS 프로비저닝 — 자격 증명 필요)** 계열과 실기기 워크스루뿐이다.
