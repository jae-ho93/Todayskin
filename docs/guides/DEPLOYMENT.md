...
**앱 롤백과 DB 스키마 롤백은 별개다 (N59).** 이전 이미지로 되돌리는 것과 DB 스키마를
되돌리는 것은 다른 문제다. 아래 규칙을 지키지 않으면 롤백 후에도 이전 코드가 새
스키마를 기대하지 못해 깨질 수 있다.
...
**이미지 롤백이 안전한 조건** (`skip_migrate=true`는 아래 조건이 참일 때만):

- 직전 배포의 migration이 **비파괴적**(새 컬럼·새 테이블 추가, nullable 확장 등)이면
  이전 코드가 새 스키마에서도 정상 동작하므로 이미지 롤백이 안전하다.
- migration이 **파괴적**(컬럼 삭제, 타입/열거 변경, 데이터 이동)이었다면 이전 코드는
  이미 사라진 스키마를 기대하므로 이미지 롤백으로는 복구할 수 없다. 이 경우:
  - **forward-fix**: 파괴적 변경을 되돌리는 **새 migration**을 포함한 커밋으로 정상
    release 경로를 탄다 (Prisma는 down migration을 지원하지 않는다).
  - 긴급 데이터 복구가 필요하면 RDS snapshot에서 복구한다 (장애 대응 런북 참고).

**파괴적 변경을 피하는 expand/contract 규칙** — 스키마 변경을 3단계로 분리한다:

1. **expand**: 새 컬럼/테이블/열거값을 추가하고, 신·구 코드가 모두 읽을 수 있는 상태로
   배포한다 (새 코드는 새 필드를, 이전 코드는 기존 필드를 사용). 데이터 백필도 이 단계.
2. **deploy**: 새 앱 코드를 전체 rollout — 이 시점까지는 이전 이미지로의 롤백이 안전하다.
3. **contract**: 최소 한 배포 주기 후, 이전 코드가 더 이상 사용하지 않는
   컬럼/열거값/제약을 제거하는 migration을 별도 release로 배포한다.

이 규칙을 지키면 어느 시점에서든 이미지 롤백이 가능해져, 앱 롤백과 스키마 변경이 서로
깨지지 않는다. 스키마를 되돌려야 하면 skip_migrate 없이, 되돌리는 **새 migration**을
포함한 커밋으로 정상 release 경로를 탄다. RDS snapshot 복구는 장애 대응 런북으로 별도
수행한다.
