...
웹 브라우저나 시뮬레이터로 확인할 때는 복사된 `localhost` 주소를 그대로 사용합니다.
실제 휴대폰으로 확인할 때만 아래 안내에 따라 LAN IP로 바꾸세요.
...
1. **회원가입** — 전화번호 입력 (`OTP_ALLOWLIST_PHONES`에 넣은 번호) → "문자 인증 시작하기"
   ...
   - **"인증하기"**를 누르면 수신 번호와 인증코드가 채워진 문자 앱이 열립니다. 문자를 발송하고 앱으로 돌아오면 자동으로 확인됩니다
   - 개발 허용 번호에서는 화면에 고정 코드 `123456`이 표시됩니다
2. **이름·생년월일 입력** → 가입 완료
