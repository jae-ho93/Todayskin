...
지금은 모든 지표가 `AirStatus` 하나(`good` / `moderate` / `bad`)를 공유하고, 프론트가 이를
좋음 / 보통 / 나쁨으로 표기한다. 지표 성격이 다른데 어휘가 하나뿐이라 틀린 말이 나온다.
실측에서 **자외선지수 9가 `bad`(= "나쁨")로 표기**됐는데, 기상청 기준으로 9는 "매우높음"이다.
자외선은 좋고 나쁨이 아니라 **높고 낮음**으로 말해야 한다.

공식 기준은 이렇다.

- 자외선지수(기상청): 낮음 · 보통 · 높음 · 매우높음 · 위험 (5단계)
- 미세먼지·초미세먼지·오존·통합대기환경지수(에어코리아): 좋음 · 보통 · 나쁨 · 매우나쁨 (4단계)

- [ ] `weather/policies/weather-status.policy.ts`의 판정을 지표별 등급 체계로 나눈다. 자외선은 5단계, 대기질 4종은 4단계. 현재 3단계로 뭉쳐 있어 "매우나쁨"과 "위험"을 표현할 수 없다
- [ ] DTO 타입을 지표별로 좁힌다(예: `UvLevel` / `AirLevel`). 문자열 하나로 합치면 프론트가 다시 지표를 보고 분기해야 하므로, 타입으로 갈라 컴파일러가 라벨 누락을 잡게 한다
- [ ] 경계값 테스트를 등급 수만큼 확장한다. R5에서 오존 경계를 `>=`에서 `>`로 고친 것처럼 경계 하나가 곧 오표기다
- [ ] OpenAPI 스펙을 다시 내보내 프론트 타입을 갱신한다 (`npm run openapi:export --prefix backend && npm run openapi:types`). R28 드리프트 검사가 CI에서 막는다
- [ ] 등급 수가 늘면 색상 단계도 늘어난다 — 색상은 프론트 소관이므로 F64와 배포 시점을 맞춘다

완료 기준: 자외선 9가 "매우높음"으로, 미세먼지 최악 구간이 "매우나쁨"으로 표기되고, 경계값 테스트가 전 등급을 덮는다.

### N41. 측정소 조회 실패가 엉뚱한 구 이름으로 표시된다 (2026-08-13 실기기)

원인 확정 · 코드 수정 · AWS 무관

해운대구에서 촬영했는데 기록에 **부산광역시 중구**로 뜬다. DB를 보면 좌표는 해운대가 맞는데
구 이름만 틀렸다.

```text
진단 스냅샷: 좌표=35.16526, 129.16350 (해운대)  →  districtName = 중구
같은 좌표로 지금 조회: districtName = 해운대구  (5회 반복 모두 동일)
```

**추가 실측(2026-08-13, 재현 완료).** 10분 뒤 같은 좌표가 다시 `중구`로 굳었고, 10회 연속
같은 값이 나왔다. 그런데 같은 시각 에어코리아 근접측정소 API를 직접 호출하면 정상이다.

```text
정부 API 직접 호출 → HTTP 200, resultCode=00
  좌동 0.9km / 우동 2.1km / 재송동 4.8km  (모두 "부산 해운대구")

우리 백엔드 /weather:
  35.17,129.16 (캐시 적중)  → district=중구      pm10=8   ← 10회 모두 동일
  35.17,129.17 (캐시 미스)  → district=해운대구  pm10=13
  35.18,129.18 (캐시 미스)  → district=해운대구  pm10=13
  35.19,129.19 (캐시 미스)  → district=해운대구  pm10=13
```

즉 조회 자체는 안정적으로 잘 된다. **문제는 한 번 실패해서 만들어진 오답이 캐시에 들어가
계속 재사용되는 것이다.** 캐시 키는 좌표를 소수 2자리로 뭉치므로(`weather-cache.ts:21`)
해운대 일대가 한 키를 공유하고, 그 키가 오염되면 그 지역 사용자 전원이 `중구`를 보게 된다.
폴백이 만든 그럴듯한 오답이 캐시와 스냅샷 두 곳에 굳는 구조다.

구 이름은 `weather.service.ts:217`에서 정해진다.

```text
districtName = nearest?.districtName ?? region.airkoreaStationName;
```

`nearest`는 에어코리아 근접측정소 조회 결과다. 이 호출이 실패하면 `null`이 되고,
근사표의 대표 구로 폴백한다. 부산광역시의 대표 구는 `'중구'`다(`regions/region.registry.ts:21`).
**즉 "구를 알 수 없다"가 "중구"라는 그럴듯한 오답으로 바뀐다.** 사용자는 둘을 구별할 수 없고,
그 값이 스냅샷에 영구 저장돼 기록에 남는다.

같은 함수에 두 번째 결함이 있다. `collectWeather()`의 `meta` 분기(스케줄러 수집 경로)는
`districtName`을 아예 설정하지 않아 `null`로 남는다. 실제로 DB의 이전 스냅샷 다수가 `districtName=null`이다.

- [ ] 측정소 조회 실패 시 근사표 대표 구로 폴백하지 않는다. 모르면 `null`로 두고 화면은 시/도만 보여준다 — 틀린 구 이름보다 낫다
- [ ] 폴백이 발생한 사실을 로그로 남긴다. 지금은 조용히 대체되어 실패율을 알 수 없다
- [ ] 실패 원인을 확인한다. 키는 설정돼 있고 지금은 성공하므로 일시 실패(타임아웃 8초 / 호출 한도)가 의심된다. 재시도 여부를 판단한다
- [ ] `meta` 분기에서도 `districtName`을 채운다. 세 분기가 같은 필드 집합을 만들도록 반환 타입을 조여 누락 시 컴파일 에러가 나게 한다
- [ ] `regionName`이 `'부산'` / `'부산광역시'`로 섞여 저장된 것도 정리한다(DB 실측). 표기가 흔들리면 지역별 집계가 갈라진다
- [ ] **폴백으로 만든 값은 캐시에 넣지 않는다.** 지표 하나가 degraded면 TTL을 30초로 줄이는 규칙은 이미 있는데(`DEGRADED_CACHE_TTL_SECONDS`), 구 이름 폴백은 degraded로 취급되지 않아 정상 TTL로 저장된다. 폴백 발생을 degraded 판정에 포함시킨다

완료 기준: 측정소 조회가 실패하면 구 이름이 비고, 성공하면 실제 구가 저장된다. 스케줄러 경로 스냅샷도 구 이름을 갖는다.
