...
> **보관 문서.** 완료된 Task 이력과 계약 기록을 보존한다.
> 활성 작업 보드는 [`docs/BACKEND_TASKS.md`](BACKEND_TASKS.md), 리팩토링 제안은
> [`docs/REFACTORING_BACKLOG.md`](REFACTORING_BACKLOG.md)를 사용한다.
> 협업 규칙은 `CONTRIBUTING.md`, 아키텍처 원칙은 `docs/ARCHITECTURE.md`가 기준이다.
...
>
> **리팩토링 백로그:** [`docs/REFACTORING_BACKLOG.md`](REFACTORING_BACKLOG.md)에 R1~R35 제안이 정리돼 있다. 승인 전 구현 금지.
...
> **N16 (AWS 첫 배포)는 Open — 활성 보드 [`BACKEND_TASKS.md`](BACKEND_TASKS.md) 참고** (완료 이력에서 제외).
