...
### F22. 내 정보 화면 — 신규 (2026-08-12, 설정 개선)
...
- [ ] 프로필 카드 전체를 탭 영역으로 → 별도 **"내 정보" 화면** 이동 (모달 제거)
- [ ] 내 정보 화면: 이름·전화(마스킹)·생년월일·성별 표시
- [ ] 이름/성별은 화면에서 수정 (`PATCH /me`), 전화 변경은 OTP 흐름 연결(추후)
- [ ] 소셜 연동 행은 계정 섹션 유지

완료: 설정에서 내 정보를 카드 탭으로 열고 항목별 수정.

### F20. 인증 화면 한 화면 리디자인 — 제안 (미확정)

- [x] 로그인: 스크롤 없는 한 화면 (헤드라인 + 전화 CTA + 소셜 아이콘 원형 48px)
- [x] 회원가입 2단계 분리: ① 전화번호+OTP ② 이름·생년월일·성별 (당근 패턴)
- [x] 소셜 버튼 풀폭 텍스트 → 아이콘 원형 48px × 3 (토스/배민 스타일)

완료 기준: 로그인/회원가입에 스크롤·고정푸터 분리 없음. (제품 확정 후 별도 진행)
